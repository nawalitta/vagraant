...
class ActiviteFixeeController extends Controller
{
    ...
    public function indexAction(Request $request)
    {
        ...
        $listActiviteFixeesId = $request->get('idActivitesFixee');
        if ($listActiviteFixeesId != null) {
            foreach ($listActiviteFixeesId as $id) {
                $activiteFixees = $activiteFixeeRepository->findOneById($id);
                if ($activiteFixees != null) {
                    $entityManager->remove($activiteFixees);
                }
                try {
                    $entityManager->flush();
                } catch (Exception $ex) {
     ...
     */
    public
    function editAction($idFixee = null, $idActivite, Request $request)
    {
        $entityManager = $this->getDoctrine()->getManager();
        $activiteFixeeRepository = $entityManager->getRepository("ActiviteBundle:ActiviteFixee");
